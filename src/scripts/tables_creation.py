...
from models import Ficha, FichaRegex, FichaQueries, Observatorio
...
def insert_fichas(table_name: str):
    ...
    for code, metadata in info_obs.items():
        ...
        try:
            ...
            #logging.error(f"Check validation for {code}: {e}")
    ...
    logging.info(f"Se insertaron {len(fichas_validated)} fichas en la tabla {table_name}")
    logging.info(f"Fichas no validadas: {len(fichas_not_validated)}")
...
def join_tables(new_table: str, fichas_table: str = "info_fichas", vistas_table: str = "vistas", key: Literal["inner", "left"] = "inner"):
    cursor, conn = connect()
    queries = FichaQueries(new_table)
    cursor.execute(queries.drop_table)
    cursor.execute(queries.join(new_table, fichas_table, vistas_table, key))
    conn.commit()
    logging.info(f"Se creó la tabla '{new_table}' a partir de un {key} join entre '{vistas_table}' y '{fichas_table}'")
...
if __name__ == "__main__":
    logging.info("==" * 50)
    #obtain_duplicates()
    #delete_table("fichas_clean")

    # insert_fichas_raw()
    insert_fichas("info_fichas")
    join_tables("fichas_clean", fichas_table="info_fichas", vistas_table="vistas")
    add_rubro_subrubro("fichas_clean")
    #exportar_tabla("fichas_clean", "SELECT * FROM fichas_clean WHERE estado != 'INACTIVO'", export_name="prueba")

    #validate_codes(table_name="fichas_clean")
